...
public class WearListCallListenerService extends WearableListenerService implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, MessageApi.MessageListener {
    ...
    public void onMessageReceived(MessageEvent messageEvent) {
        ...
        if(messageEvent.getPath().contains("/get_history")) {
            ...
            currentAddress = RecryptSharedPreference.getInstance().getCurrentAddress(getApplicationContext());
            if (RecryptApplication.instance == null || RecryptApplication.instance.getWearableMessagingProvider() == null) {
                upSocketService();
            } else {
                sendData();
            }
    ...
    private void sendData() {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                histories = RecryptApplication.instance.getWearableMessagingProvider().getOperations();
                String balance = RecryptApplication.instance.getWearableMessagingProvider().getBalance();
                String uncBalance = RecryptApplication.instance.getWearableMessagingProvider().getUnconfirmedBalance();
                String address = RecryptApplication.instance.getWearableMessagingProvider().getAddress();
                Gson gson = new Gson();
