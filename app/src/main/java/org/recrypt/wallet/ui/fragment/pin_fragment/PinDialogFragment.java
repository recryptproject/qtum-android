...
import org.recrypt.wallet.utils.PinEntryEditText;
...
public class PinDialogFragment extends DialogFragment {
    ...
    @BindView(org.recrypt.wallet.R.id.til_wallet_pin)
    PinEntryEditText mWalletPin;

    @BindView(org.recrypt.wallet.R.id.tv_toolbar_title)
    FontTextView mTextViewToolBarTitle;

    @BindView(org.recrypt.wallet.R.id.tooltip)
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        View view = LayoutInflater.from(getActivity()).inflate(ThemeUtils.getCurrentTheme(getContext()).equals(ThemeUtils.THEME_DARK) ? org.recrypt.wallet.R.layout.dialog_fragment_pin : org.recrypt.wallet.R.layout.dialog_fragment_pin_light, null);
    ...
    private String getSixDigitPassword() {
        String encryptedPinHash = RecryptSharedPreference.getInstance().getSixDigitPassword(getContext());
    ...
    private String getFourDigitPassword() {
        String encryptedPinHash = RecryptSharedPreference.getInstance().getPassword(getContext());
        return KeyStoreHelper.decrypt(RECRYPT_PIN_ALIAS, encryptedPinHash);
    }
            ...
            ((MainActivity) getActivity()).checkSensorState(new MainActivity.SensorStateListener() {
                ...
                public void onRequest(MainActivity.SensorState sensorState) {
                    if (sensorState == MainActivity.SensorState.READY) {
                        mTextViewToolBarTitle.setText(org.recrypt.wallet.R.string.confirm_fingerprint_or_pin);
                        prepareSensor();
                    } else {
                        mTextViewToolBarTitle.setText(org.recrypt.wallet.R.string.enter_pin);
                    }
                }
            });
        } else {
    ...
    private void confirmError(String errorText) {
        if (getDialog() != null) {
            ...
            tooltip.setTextColor(ContextCompat.getColor(getContext(), org.recrypt.wallet.R.color.accent_red_color));
    ...
    private class FingerprintHelper extends FingerprintManagerCompat.AuthenticationCallback {
        ...
        public void onAuthenticationSucceeded(FingerprintManagerCompat.AuthenticationResult result) {
            ...
            String encoded = RecryptSharedPreference.getInstance().getTouchIdPassword(getContext());
    ...
    private void confirm(String pin) {
        Long banTime = RecryptSharedPreference.getInstance().getBanTime(getContext());
        ...
        if (isCorrect) {
            ...
            confirmError(getContext().getString(org.recrypt.wallet.R.string.incorrect_pin));
        }
    }
    ...
    private void changeBanState(boolean isCorrect) {
        ...
        if (isCorrect) {
            ...
            failedAttemptsCount = RecryptSharedPreference.getInstance().getFailedAttemptsCount(getContext());
            failedAttemptsCount++;
            if (failedAttemptsCount == 3) {
                Long currentTime = Calendar.getInstance().getTimeInMillis();
                Long banTime = currentTime + mBanTime;
                RecryptSharedPreference.getInstance().setBanTime(getContext(), banTime);
                failedAttemptsCount = 0;
                mPinCallBack.onError(getString(R.string.sorry_please_try_again_in) + " " + mBanTime.intValue() / 60000 + " " + getString(R.string.minutes));
            }
        }
        RecryptSharedPreference.getInstance().setFailedAttemptsCount(getContext(), failedAttemptsCount);
    }
