...
import org.recrypt.wallet.WearableMessagingProvider;
...
import org.recrypt.wallet.dataprovider.services.update_service.WatchUpdateService;
...
import org.recrypt.wallet.ui.activity.splash_activity.SplashActivity;
import org.recrypt.wallet.ui.base.base_activity.BaseActivity;
...
import org.recrypt.wallet.ui.fragment.pin_fragment.PinAction;
...
import org.recrypt.wallet.utils.CustomContextWrapper;
import org.recrypt.wallet.utils.FontManager;

import org.recrypt.wallet.ui.fragment.profile_fragment.ProfileFragment;
import org.recrypt.wallet.ui.fragment.send_fragment.SendFragment;
...
public class MainActivity extends BaseActivity implements MainActivityView, WearableMessagingProvider {
        ...
        mServiceConnection = new ServiceConnection() {
            ...
            public void onServiceConnected(ComponentName componentName, IBinder iBinder) {
                if(iBinder instanceof UpdateService.UpdateBinder) {
                    RecryptApplication.instance.setWearableMessagingProvider(MainActivity.this);
    ...
    protected void onNewIntent(Intent intent) {
        ...
        switch (intent.getAction()) {
            case RecryptIntent.OPEN_FROM_NOTIFICATION:
    ...
    public boolean checkTouchIdEnable() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (RecryptSharedPreference.getInstance().isTouchIdEnable(getContext())) {
    ...
    public void initializeViews() {
        ...
        switch (intent.getAction()) {
            case RecryptIntent.SEND_FROM_SDK:
                getPresenter().setSendFromIntent(true);
                mAddressForSendAction = intent.getStringExtra(RecryptIntent.SEND_ADDRESS);
                mAmountForSendAction = intent.getStringExtra(RecryptIntent.SEND_AMOUNT);
                mTokenAddressForSendAction = intent.getStringExtra(RecryptIntent.SEND_TOKEN);
                break;
    ...
    protected void attachBaseContext(Context newBase) {
        super.attachBaseContext(CustomContextWrapper.wrap(newBase, RecryptSharedPreference.getInstance().getLanguage(newBase)));
    }

    public RecryptApplication getRecryptApplication() {
        return (RecryptApplication) getApplication();
    }
